...
- `dotnet-code-reviewer` — .NET / C# 9 / Clean Arch / **SOLID** / **YAGNI** 코드 리뷰 (SOLID·YAGNI 명확 위반은 `high` → `REQUEST_CHANGES` 로 머지 게이트. YAGNI 는 단일 사용 추상화·미사용 설정·요청되지 않은 유연성·불가능 케이스 방어 코드를 차단하되, `IApplicationDbContext`·`IRequest`/`IRequestHandler`·CQRS 슬라이스 계약 등 **필수 구조는 예외**)
