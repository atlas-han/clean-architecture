...
- **2026-06-24 — Wave 3 실행 완료 (F2·F3, 옵션 F9·F10 보류)**: 데모와 무관한 DRY 추출 —
  F2 `DbSet<T>.FindOrThrowAsync<T>(id, ct)` 확장(`Application/Common/Extensions`)을 신설해 Cancel/Confirm/Update/Delete
  4개 핸들러의 "로드-or-NotFound" 가드를 단일화(+헬퍼 단위테스트 2건). `IApplicationDbContext` 인터페이스는 건드리지
  않아 `TestDbContext`/`ApplicationDbContext` 무변경. (CreateOrder/PlaceOrder의 per-line `Product` 조회는
  루프 안 `line.ProductId` 조회이고 데모 슬라이스라 의도적으로 제외 — 헬퍼 누락이 아님.)
  F3 `ProductValidationRules`(`Common/Validation`) FluentValidation `IRuleBuilder` 확장으로 Create/Update validator의
  Name/Description/Price/Stock 규칙을 공유 — 슬라이스별 Validator 클래스는 유지하고 규칙만 공유, Update의 메시지
  드리프트(누락된 `.WithMessage`)도 교정. F9(컨트롤러 `CreatedFrom` 헬퍼)·F10(페이징 확장)은 저가치(YAGNI)로 **보류**.
  상태는 Wave 4 + F11(ii) 미결로 `Proposed` 유지.
